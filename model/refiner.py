...
class Refiner(nn.Module):
    ...
    Input:
        src_bgr: (B, 6, H, W) full resolution src and bgr image concatenated
        ...
        hid: (B, 16, Hc, Hc) coarse hidden encoding.
    ...
    def forward(self,
        ...
        if self.mode != 'full':
            ...
            pha = x[:, :1]
            fgr = x[:, 1:]
            ref = torch.ones((src_bgr.size(0), 1, H_quat, W_quat), device=src_bgr.device, dtype=src_bgr.dtype)
